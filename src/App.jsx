...
import { 
  ...
  Video as VideoIcon, DollarSign, Edit, Share2, Medal, BarChart3
} from 'lucide-react';
...
// --- Sync Hook (Socket.IO + LocalStorage + CDN Loader) ---
  ...
  useEffect(() => {
    ...
    socket.on('init_state', (allData) => {
        if (allData && allData[key] !== undefined) {
            ...
            window.localStorage.setItem(key, JSON.stringify(allData[key]));
        }
    });
...
function RefereeManager({ onBack }) {
    const [referees, setReferees] = useSyncedState('volleyball_referees', []);
...
function TeamManager({ onBack }) {
  const [teams, setTeams] = useSyncedState('volleyball_teams', []);
...
function Dashboard({ onControl, onOutput, onStadium, onManageTeams, onManageReferees }) {
  const [matches, setMatches, status] = useSyncedState('volleyball_matches', []);
  const [teams] = useSyncedState('volleyball_teams', []);
  const [referees] = useSyncedState('volleyball_referees', []);
  ...
  const createMatch = (e) => {
      ...
      const newMatch = {
          ...
          activeReferee: 1, // Default to Ref 1
          serving: 'A',
          serveVisible: false, // Default Serve Indicator OFF
          activeText: '',
          activePlayerId: null,
          activeView: 'scoreboard',
          graphicsVisible: false,
          setsVisible: true, // Default ON
          setHistory: [],
          lineupStep: 0,
          isSwapped: false,
          ledSponsors: [],
      ...
      setMatches([...matchList, newMatch].sort((a,b) => new Date(a.date) - new Date(b.date)));
      setIsCreating(false);
  };
        ...
        <div className="h-screen flex flex-col bg-slate-100">
            <div className="bg-slate-800 text-white p-4 flex justify-between items-center shadow-lg">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button 
                        onClick={cycleStatus} 
                        className={`text-xs px-4 py-2 rounded font-black uppercase tracking-wider transition-all ${
                            match.status === 'Live' ? 'bg-red-600 animate-pulse shadow-red-500/50 shadow-lg' : 
                            ...
                            <div className="flex-1 flex gap-4 overflow-hidden">
                                {[currentTeamA, currentTeamB].map((t, i) => (
                                    <div key={i} className="flex-1 overflow-y-auto">
                                        <div className={`text-xs font-bold uppercase mb-2 ${i===0?'text-blue-600':'text-red-600'}`}>{t.name} ({(i===0?match.lineupA:match.lineupB)?.length}/6)</div>
                                        <div className="space-y-1">{(t.roster || []).map(p => { const list = i===0 ? match.lineupA : match.lineupB; const field = i===0 ? 'lineupA' : 'lineupB'; const checked = list?.includes(p.id); return ( <label key={p.id} className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded border cursor-pointer"><input type="checkbox" checked={checked} onChange={()=>{ const newList = checked ? list.filter(x=>x!==p.id) : [...(list||[]), p.id]; if(newList.length <= 6) updateMatch({[field]: newList}); }}/><span className="font-mono bg-slate-100 px-1 rounded text-xs">#{p.number}</span><span className="text-sm font-bold">{p.name}</span></label> )})}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {isSquare ? (
                    ...
                    <div className="relative z-10 aspect-square h-full max-h-screen flex flex-col gap-2 bg-black shadow-2xl border border-white/5 p-4">
                              ...
                              {match.showLedSponsors && (
                                  <div className="h-[95%] w-[40%] flex items-center justify-center bg-white rounded-lg p-1 z-10 shadow-sm">
                                      {match.ledSponsors?.length > 0 ? (
                                           <img key={sponsorIdx} src={match.ledSponsors[sponsorIdx]} className="w-full h-full object-contain animate-fade-cycle" />
                         ...
                         <div className="h-[50%] w-full flex gap-2">
                             ...
                             <div className="flex-1 bg-slate-900 rounded-xl border-l-8 border-t border-b border-r border-slate-700 flex flex-col items-center p-2 relative shadow-lg overflow-hidden" style={{ borderLeftColor: left.color }}>
                                 ...
                                 <div className="h-[30%] w-full flex justify-center mt-2">
                                     <div className="aspect-video h-full bg-black border border-slate-600 shadow relative">
                                         {left.flag ? <img src={left.flag} className="w-full h-full object-cover"/> : <Flag className="text-slate-600 m-auto"/>}
                                     </div>
                                 </div>
                             ...
                             <div className="flex-1 bg-slate-900 rounded-xl border-l-8 border-t border-b border-r border-slate-700 flex flex-col items-center p-2 relative shadow-lg overflow-hidden" style={{ borderLeftColor: right.color }}>
                                 ...
                                 <div className="h-[30%] w-full flex justify-center mt-2">
                                     <div className="aspect-video h-full bg-black border border-slate-600 shadow relative">
                                         {right.flag ? <img src={right.flag} className="w-full h-full object-cover"/> : <Flag className="text-slate-600 m-auto"/>}
                                     </div>
                                 </div>
                         ...
                         <div className="h-[35%] w-full bg-slate-900 rounded-xl border border-slate-700 flex flex-col shadow-inner overflow-hidden">
                             ...
                             <div className="flex-1 flex items-center border-b border-slate-700/50">
                                 <div className="w-24 h-full p-2 flex items-center justify-center bg-black/20 border-r border-slate-700">
                                      {left.flag ? <img src={left.flag} className="w-full h-full object-contain"/> : <Flag className="text-slate-600"/>}
                                 </div>
                                 {[1,2,3,4,5].map(i => { const result = getSetResult(left, i); return (
                             ...
                             <div className="flex-1 flex items-center">
                                 <div className="w-24 h-full p-2 flex items-center justify-center bg-black/20 border-r border-slate-700">
                                      {right.flag ? <img src={right.flag} className="w-full h-full object-contain"/> : <Flag className="text-slate-600"/>}
                                 </div>
                                 {[1,2,3,4,5].map(i => { const result = getSetResult(right, i); return (
                                     <div key={i} className={`flex-1 h-full flex items-center justify-center font-black text-6xl border-l border-slate-700/50 ${result.isWinner ? 'text-yellow-400 bg-yellow-900/10' : 'text-white/50'}`}>
                                         {result.score}
                                     </div>
                                 ) })}
                ...
                <>
                    ...
                    <div className="relative z-10 flex-1 flex items-stretch justify-between gap-4 px-8 pt-24">
                        <div className="flex-1 bg-slate-900/50 rounded-3xl border-4 border-slate-700 flex flex-col items-center justify-between p-6 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-sm">
                             <div className="w-full flex flex-col items-center gap-2">
                                 <div className="h-40 w-64 bg-black rounded-xl overflow-hidden border-4 border-slate-600 shadow-lg relative">{left.flag ? <img src={left.flag} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-slate-700"><Flag size={64}/></div>}</div>
                        ...
                        <div className="flex-1 bg-slate-900/50 rounded-3xl border-4 border-slate-700 flex flex-col items-center justify-between p-6 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-sm">
                             <div className="w-full flex flex-col items-center gap-2">
                                 <div className="h-40 w-64 bg-black rounded-xl overflow-hidden border-4 border-slate-600 shadow-lg relative">{right.flag ? <img src={right.flag} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-slate-700"><Flag size={64}/></div>}</div>
